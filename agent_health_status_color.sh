...
${verbose} && printf "${WARN}DS Policy Id - ${dsPolicyId}${NC}\n";
...
if [[ $hasDSA != 1 ]]; then
    ...
    if [[ -z "${dsApiKey}" ]]; then
        printf "${ERR}Api Key parameter was not passed when running this script. Retry this script with: sudo ./agent_health_status.sh <your-api-key>.${NC}\n";
